...
public class ScanServersPage {
    ...
    public static String renderForm(long userId, String message, String error) {
        ...
        String content = HtmlTemplate.statsHeader(userId)
                ...
                + "<p class='text-sm text-slate-400 mt-2'>Enter a directory to scan for Minecraft server installations.</p>"
                + "</div>"
    ...
    public static String renderResults(long userId, String scanDir, List<ScannedServer> servers, String message, String error) {
        String messageBlock = banner(message, "emerald");
        String errorBlock = banner(error, "rose");
        ...
        for (int i = 0; i < servers.size(); i++) {
            ...
            String dashBadge = s.dashInstalled
                    ? "<span class='px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400 text-[11px] font-bold'>NeoBridge Installed</span>"
                    : "<span class='px-2 py-0.5 rounded-full bg-slate-600/40 text-slate-400 text-[11px] font-bold'>No NeoBridge</span>";

            cards.append("<article class='relative rounded-2xl bg-[#1e293b] border border-slate-700/70 shadow-xl p-5 flex flex-col gap-3'>");
            ...
            cards.append(formInputNumber("Dash Plugin Port", "dash_port", s.dashPort > 0 ? s.dashPort : 8080));
        ...
        String content = HtmlTemplate.statsHeader(userId)
                ...
                + "<p class='text-slate-400 mt-1'>Found <strong class='text-white'>" + servers.size() + "</strong> server(s) in <span class='font-mono text-cyan-300'>" + esc(scanDir) + "</span></p></div>"
