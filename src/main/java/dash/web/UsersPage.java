...
public class UsersPage {
    ...
    public static String render(long currentUserId, Map<String, WebAuth.UserInfo> users, Iterable<String> roles, Map<String, Integer> roleValues,
        ...
        return render(currentUserId, users, roles, roleValues, actorUsername, actorIsMainAdmin,
                generatedCode, message, pendingRegistrations, availableServers, assignmentsByUser,
                List.of());
    }
    ...
    public static String render(long currentUserId, Map<String, WebAuth.UserInfo> users, Iterable<String> roles, Map<String, Integer> roleValues,
            ...
            List<DatabaseManager.ServerRecord> availableServers,
            Map<String, List<DatabaseManager.UserServerAssignment>> assignmentsByUser,
            List<WebAuth.UserInfo> pendingBridgeUsers) {
        ...
        for (WebAuth.UserInfo user : users.values()) {
            ...
            String bridgeBadge = user.bridgeUser()
                    ...
                    + (user.bridgeApproved()
                            ? "bg-emerald-500/20 text-emerald-300 border border-emerald-500/40'>SSO"
                            : "bg-amber-500/20 text-amber-300 border border-amber-500/40'>SSO Pending")
                    + "</span>"
                    : "";

            String transferMainAdminForm = "";
            if (actorIsMainAdmin && !user.mainAdmin()) {
                transferMainAdminForm = "<form action='/users/transfer-main-admin' method='post' class='mt-3' "
                        + "onsubmit=\"return confirm('Transfer Main-Admin role to " + userName
                        + "? You will become a normal admin.');\">"
                        + "<input type='hidden' name='username' value='" + userName + "'>"
                        + "<button type='submit' class='w-full sm:w-auto rounded-lg px-4 py-2 border border-amber-500/60 text-amber-300 hover:bg-amber-500/10 text-sm font-semibold transition-colors'>"
                        + "Make Main-Admin</button>"
                        + "</form>";
            }
            ...
            usersGrid.append("<article class='rounded-2xl bg-[#1e293b] border border-slate-700/70 p-5 shadow-lg'>")
                    ...
                    .append("</div>")
                    .append("</div>")
                    .append("<form action='/users/update-role' method='post' class='space-y-3'>")
        ...
        String content = HtmlTemplate.statsHeader(currentUserId)
                ...
                + "</form>"
                + "</section>"
                + renderPendingBridgeUsersSection(pendingBridgeUsers, actorIsMainAdmin, roleList)
                + "<section>"
    ...
    private static String renderServerAssignmentSection(WebAuth.UserInfo user, boolean canManageThisUser,
        ...
        if (assignments.isEmpty()) {
            ...
            for (DatabaseManager.UserServerAssignment assignment : assignments) {
                assigned.append("<div class='flex items-center justify-between gap-2 rounded-lg bg-[#0f172a] border border-slate-700/60 px-3 py-2'>")
                        ...
                        .append("</div>")
                        .append("<form action='/users/revoke-server' method='post'>")
        ...
        return "<div class='mt-4 pt-4 border-t border-slate-700/60'>"
                ...
                + "<form action='/users/assign-server' method='post' class='grid grid-cols-1 md:grid-cols-[1fr_110px] gap-2'>"
    ...
    private static String renderPendingBridgeUsersSection(List<WebAuth.UserInfo> pendingBridgeUsers,
            boolean actorIsMainAdmin, List<String> roles) {
        if (pendingBridgeUsers == null || pendingBridgeUsers.isEmpty() || !actorIsMainAdmin) {
            return "";
        }

        StringBuilder roleOpts = new StringBuilder();
        for (String role : roles) {
            roleOpts.append("<option value='").append(escapeHtml(role)).append("'>")
                    .append(escapeHtml(role)).append("</option>");
        }

        StringBuilder cards = new StringBuilder();
        for (WebAuth.UserInfo pending : pendingBridgeUsers) {
            String name = escapeHtml(pending.username());
            cards.append("<div class='rounded-xl bg-[#0f172a] border border-amber-500/30 p-4'>")
                    .append("<div class='flex items-center justify-between mb-3'>")
                    .append("<span class='text-white font-semibold'>").append(name).append("</span>")
                    .append("<span class='px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase bg-amber-500/20 text-amber-300 border border-amber-500/40'>SSO Pending</span>")
                    .append("</div>")
                    .append("<form action='/bridge-users/approve' method='post' class='flex flex-col sm:flex-row items-end gap-2 mb-2'>")
                    .append("<input type='hidden' name='username' value='").append(name).append("'>")
                    .append("<select name='role' class='flex-1 bg-[#1e293b] border border-slate-600 rounded-lg px-3 py-2 text-xs text-slate-100'>")
                    .append(roleOpts)
                    .append("</select>")
                    .append("<button type='submit' class='rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-semibold px-4 py-2 transition-colors'>Approve</button>")
                    .append("</form>")
                    .append("<form action='/bridge-users/deny' method='post' onsubmit=\"return confirm('Deny and remove this bridge user?');\">")
                    .append("<input type='hidden' name='username' value='").append(name).append("'>")
                    .append("<button type='submit' class='rounded-lg border border-rose-500/50 text-rose-300 hover:bg-rose-500/10 text-xs font-semibold px-4 py-2 transition-colors'>Deny</button>")
                    .append("</form>")
                    .append("</div>");
        }

        return "<section class='rounded-2xl bg-[#1e293b] border border-amber-500/30 p-6 shadow-lg'>"
                + "<h2 class='text-lg font-semibold text-amber-300 mb-4'>Pending Bridge Users (SSO)</h2>"
                + "<p class='text-xs text-slate-400 mb-4'>These users signed in via SSO from a bridge server and are awaiting your approval.</p>"
                + "<div class='grid grid-cols-1 md:grid-cols-2 gap-4'>"
                + cards
                + "</div>"
                + "</section>";
