...
public class InstallServerPage {
    ...
    public static String render(long currentUserId, String message, String error, String warning) {
        ...
        String content = HtmlTemplate.statsHeader(currentUserId)
                ...
                + inputField("Install Dir", "install_dir", "/home/mc/servers/paper-1", true)
    ...
    private static String installProgressScript() {
        return "<script>"
                ...
                + ".then(d=>{if(!d||!Array.isArray(d.lines)){return;} out.textContent=d.lines.join('\\n')||'Preparing install...'; out.scrollTop=out.scrollHeight;})"
                + ".catch(()=>{});"
                + "}"
                ...
                + "console.log('Sending Encoded Data:', data.toString());"
                ...
                + "if(resp.ok){if(poller){clearInterval(poller);} if(resp.redirected){window.location.href=resp.url; return;} window.location.reload(); return;}"
                ...
                + "showError(msg);"
                + "})"
                + ".catch(function(){resetInstallUi(false); const msg='Installation request failed. Please retry.'; out.textContent+='\\n'+msg; showError(msg);});"
                + "});"
                + "})();"
                + "</script>";
    }
    ...
    private static String bridgeCompatibilityScript() {
        return "<script>"
                ...
                + "const bridgeAllowed=(type==='PAPER'||type==='PURPUR'||type==='SPIGOT'||type==='BUKKIT');"
