...
public class CreateServerPage {
    ...
    public static String render(long currentUserId, String message, String error) {
        ...
        String content = HtmlTemplate.statsHeader(currentUserId)
                ...
                + numberField("Dash/FabricDash Port", "dash_port", "8080")
                + bridgePortField("Bridge Port", "bridge_port")
                + passwordField("Bridge Secret", "bridge_secret", "Bridge Secret")
                + toggleField("Enable Dash/FabricDash Bridge", "use_plugin_interface")
    ...
    private static String toggleField(String label, String name) {
        return "<div class='rounded-xl border border-slate-700/50 bg-[#0f172a] px-4 py-3'>"
                ...
                + "</label>"
                + "<p class='mt-2 text-xs text-slate-500'>Enable only if Dash or FabricDash is installed on this server.</p>"
                + "</div>";
    }
