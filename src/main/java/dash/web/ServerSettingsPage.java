...
public class ServerSettingsPage {
    ...
    public static String render(long currentUserId, long serverId, DatabaseManager.ServerRecord server, boolean success, String errorMessage,
        ...
        String bridgeHint = "Shared secret used to sign SSO redirects to Dash/FabricDash.";
        ...
        String content = HtmlTemplate.statsHeader(currentUserId)
                ...
                + field("Dash/FabricDash Port", "dash_port", "number", dashPortValue, "Dash/FabricDash web + SSO port on the target server", true)
                ...
                + toggleField("Use Plugin Interface (Dash/FabricDash)", "use_plugin_interface", server.usePluginInterface(),
                        "When enabled, the homescreen routes this server to the bridge package UI; otherwise NeoDash stays on the native dashboard.")
                + "</div>"
                ...
                + "<script>"
                + "(function(){"
                + "var secretInput = document.getElementById('bridge_secret_input');"
                + "var toggle = document.getElementById('use_plugin_interface_toggle');"
                + "var hint = document.getElementById('bridge_secret_hint');"
                + "if (!secretInput || !toggle) return;"
                + "function checkSecret(){"
                + "  var secret = secretInput.value.trim();"
                + "  if (!secret) { if(hint) hint.textContent=''; return; }"
                + "  fetch('/api/check-bridge-secret?server_id=" + serverId + "&secret=' + encodeURIComponent(secret))"
                + "    .then(function(r){return r.json();})"
                + "    .then(function(d){"
                + "      if(d.matches){"
                + "        if(hint) { hint.textContent='✓ Secret matches plugin config'; hint.className='mt-1 text-xs text-emerald-400'; }"
                + "      } else {"
                + "        if(hint && secret.length > 0) { hint.textContent='Secret does not match plugin config'; hint.className='mt-1 text-xs text-amber-400'; }"
                + "      }"
                + "    }).catch(function(){});"
                + "}"
                + "secretInput.addEventListener('change', checkSecret);"
                + "secretInput.addEventListener('blur', checkSecret);"
                + "})();"
                + "</script>"
                + "</main>";
    ...
    private static String passwordField(String label, String name, String value, String help) {
        return "<div class='md:col-span-2'>"
                + "<label class='block text-xs font-medium text-slate-400 uppercase tracking-wider mb-2'>" + label + "</label>"
                + "<input type='password' id='bridge_secret_input' name='" + name + "' value='" + escapeHtml(value)
                + "' autocomplete='off'"
                + " class='w-full bg-slate-800 border border-slate-600 rounded-lg px-3 py-2 text-sm text-white font-mono focus:border-primary outline-none'"
                + " placeholder='Enter bridge secret'>"
                + "<p id='bridge_secret_hint' class='mt-1 text-xs text-slate-500'>" + escapeHtml(help) + "</p>"
                + "</div>";
    }

    private static String toggleField(String label, String name, boolean checked, String help) {
        String checkedAttr = checked ? " checked" : "";
        String idAttr = "use_plugin_interface".equals(name) ? " id='use_plugin_interface_toggle'" : "";
        return "<div class='md:col-span-2 rounded-xl border border-slate-700/50 bg-[#0f172a] px-4 py-3'>"
                ...
                + "<input type='checkbox' class='peer sr-only' name='" + escapeHtml(name) + "' value='1'" + idAttr + checkedAttr + ">"
