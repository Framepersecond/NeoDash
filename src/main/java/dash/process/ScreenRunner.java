...
public class ScreenRunner implements ServerProcessRunner {
    ...
    public void start() throws IOException {
        // Verify screen is available before attempting to launch, so the user gets a
        // clear error instead of a cryptic "No such file or directory" from the OS.
        try {
            new ProcessBuilder("screen", "--version").redirectErrorStream(true).start().waitFor();
        } catch (Exception e) {
            throw new IOException(
                    "The 'screen' utility is not installed or not on PATH. "
                    + "Install it (e.g. 'sudo apt install screen' / 'sudo pacman -S screen') "
                    + "and restart NeoDash.", e);
        }
